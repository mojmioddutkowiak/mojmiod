            ...
            {foreach from=$hook_content item=content name=myLoop}
            	<div class="item_{$smarty.foreach.myLoop.iteration|escape:'htmlall':'UTF-8'}{if $content.hook == 'home'} col-md-6 col-lg-12 col-xs-12{else} col-xs-12{/if}">
                    ...
                    {elseif $content.item_type == 'instagram' && $global_instagram}
                        ...
                        <script type="text/javascript" src="{$module_dir|escape:'html':'UTF-8'}views/js/instafeed.js"></script>
                        {if $global_instagram_type == 'tagged'}
                            <script type="text/javascript">
                                var feed = new Instafeed({
                                    get: 'tagged',
                                    target: 'instafeed_{$content.hook|escape:"html":"UTF-8"}',
                                    tagName: '{$global_instagram_tag|escape:"html":"UTF-8"}',
                                    clientId: '{$global_instagram_id|escape:"html":"UTF-8"}',
                                    limit:{$limit|escape:"html":"UTF-8"},
                                    sortBy: 'most-recent'
                                });
                                feed.run();
                            </script>
                        {else}
                            <script type="text/javascript">
                                var feed = new Instafeed({
                                    userId:{$global_instagram_userid|escape:"html":"UTF-8"},
                                    get: 'user',
                                    target: 'instafeed_{$content.hook|escape:"html":"UTF-8"}',
                                    clientId: '{$global_instagram_id|escape:"html":"UTF-8"}',
                                    accessToken: '{$global_instagram_token|escape:"html":"UTF-8"}',
                                    limit:{$limit|escape:"html":"UTF-8"},
                                    sortBy: 'most-recent'
                                });
                                feed.run();
