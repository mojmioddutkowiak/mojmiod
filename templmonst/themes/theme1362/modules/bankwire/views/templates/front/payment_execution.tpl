...
{else}
  <form action="{$link->getModuleLink('bankwire', 'validation', [], true)|escape:'html':'UTF-8'}" method="post">
    ...
    <p class="cart_navigation clearfix" id="cart_navigation">
      <a class="btn btn-default btn-md icon-left" href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html':'UTF-8'}" title="{l s='Other payment methods' mod='bankwire'}">{l s='Other payment methods' mod='bankwire'} </a>
      <button class="btn btn-default btn-md icon-right" type="submit"><span>{l s='I confirm my order' mod='bankwire'}</span></button>
    </p>
  </form>
{/if}