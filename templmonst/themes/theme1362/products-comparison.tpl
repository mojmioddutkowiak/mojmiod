          ...
          <td class="ajax_block_product comparison_infos product-block product-{$product->id}">
            ...
            <div class="prices-container">
              {if isset($product->show_price) && $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
                ...
                {if $product->on_sale}
                  {elseif $product->specificPrice && $product->specificPrice.reduction}
            ...
            <div class="comparison_product_infos">
              <p class="comparison_availability_statut">
                {if !(($product->quantity <= 0 && !$product->available_later) || ($product->quantity != 0 && !$product->available_now) || !$product->available_for_order || $PS_CATALOG_MODE)}
              ...
              <div class="clearfix">
                <div class="button-container">
                  {if (!$product->hasAttributes() || (isset($add_prod_display) && ($add_prod_display == 1))) && $product->minimal_quantity == 1 && $product->customizable != 2 && !$PS_CATALOG_MODE}
...
<ul class="footer_link">
  <li>
    <a class="btn btn-default icon-left" href="{if isset($force_ssl) && $force_ssl}{$base_dir_ssl}{else}{$base_dir}{/if}" title="{l s='Continue Shopping'}">
      <span>
