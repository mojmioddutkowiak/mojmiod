...
*}
{capture name=path}<a href="{$link->getPageLink('my-account', true)|escape:'htmlall':'UTF-8'}" title="{l s='Manage my account' mod='tmsociallogin'}" rel="nofollow">{l s='My account' mod='tmsociallogin'}</a><span class="navigation-pipe">{$navigationPipe}</span>{l s='Facebook registration' mod='tmsociallogin'}{/capture}
{include file="$tpl_dir./errors.tpl"}

<form action="{$link->getModuleLink('tmsociallogin', 'facebookregistration', array(), true)}" method="post" class="box">
	<div class="row">
