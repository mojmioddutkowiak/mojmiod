                ...
                <ul id="tmrelatedproducts" class="clearfix">
                    {foreach from=$related_products item=related name=related_products_list}
                        {if ($related.allow_oosp || $related.quantity_all_versions > 0 || $related.quantity > 0) && $related.available_for_order && !isset($restricted_country_mode)}
                            ...
                            <li class="item product-box ajax_block_product{if $smarty.foreach.related_products_list.first} first_item{elseif $smarty.foreach.related_products_list.last} last_item{else} item{/if} product_related_products_description">
                                <div class="product_desc">
                                    <a href="{$relatedLink|escape:'html':'UTF-8'}" title="{$related.legend|escape:'html':'UTF-8'}" class="product-image product_image">
                                        <img class="lazyOwl" src="{$link->getImageLink($related.link_rewrite, $related.id_image, 'home_default')|escape:'html':'UTF-8'}" alt="{$related.legend|escape:'html':'UTF-8'}" />
                                    </a>
                                    <div class="block_description">
                                        <a href="{$relatedLink|escape:'html':'UTF-8'}" title="{l s='More' mod='tmrelatedproducts'}" class="product_description">
                                            {$related.description_short|strip_tags|truncate:25:'...'|escape:'html':'UTF-8'}
                                        </a>
                                    </div>
                                </div>
                                <div class="s_title_block">
                                    <h5 class="product-name">
                                        <a title="{$related.name|escape:'html':'UTF-8'}" href="{$relatedLink|escape:'html':'UTF-8'}">
