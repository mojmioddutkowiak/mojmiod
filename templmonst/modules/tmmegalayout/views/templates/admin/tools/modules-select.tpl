...
{if isset($modules_list) && count($modules_list) > 0}
    ...
    <div class="form-group popup-content">
        ...
        <div class="form-group">
            ...
            <select data-select-id="{$id_hook|escape:'htmlall':'UTF-8'}" name="tmml_module_{$id_hook|escape:'htmlall':'UTF-8'}">
                {foreach from=$modules_list item='module'}
                    <option value="{$module.name|escape:'htmlall':'UTF-8'}">{$module.public_name|escape:'htmlall':'UTF-8'}</option>
                {/foreach}
            </select>
        </div>
    </div>
