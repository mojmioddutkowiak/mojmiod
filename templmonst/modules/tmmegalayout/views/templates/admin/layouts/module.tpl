    ...
    <article class="module-inner clearfix inner">
        ...
        <div class="button-container clearfix">
            <span {if $preview == false}data-toggle="tooltip" data-placement="right" title="{Module::getModuleName({$elem.module_name|escape:'htmlall':'UTF-8'})|escape:'htmlall':'UTF-8'}" class="module-name"{/if}>
                {if $preview == false}<span class="module-sign">{if $module_icon}<img class="img-responsive" src="{$module_icon|escape:'htmlall':'UTF-8'}" alt="{$elem.module_name|escape:'htmlall':'UTF-8'}" />{else}M{/if}</span>{/if}<span class="module-text">{Module::getModuleName({$elem.module_name|escape:'htmlall':'UTF-8'})|escape:'htmlall':'UTF-8'}</span>
            </span>
            {if $preview == false}
