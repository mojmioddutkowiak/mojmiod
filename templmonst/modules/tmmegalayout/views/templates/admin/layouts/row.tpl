    ...
    <article class="row-inner clearfix inner">
        {if $preview == false}
            <div class="button-container clearfix">
                <span class="element-name">{l s='Row' mod='tmmegalayout'} <span class="sort-order">{$elem.sort_order|escape:'htmlall':'UTF-8'}</span> <span class="identificator">{if $elem.specific_class}({$elem.specific_class|escape:'htmlall':'UTF-8'|replace:' ':' | '}){/if}</span></span>
