...
<div class="tmmegalayout-lsettins panel clearfix">
    <div class="button-container">
        <a href="#" class="btn btn-success add_layout" data-hook-id="{$content.id_hook|escape:'htmlall':'UTF-8'}">+ {l s='Add a Preset' mod='tmmegalayout'}</a>
    </div>
    <div class="tmlist-group-container dropdown">
        ...
        <ul data-list-id="{$content.id_hook|escape:'htmlall':'UTF-8'}" class="tmlist-group tmml-layouts-list dropdown-menu" aria-labelledby="dropdownMenu">
            {if isset($content.layouts_list) && $content.layouts_list}
                {foreach from=$content.layouts_list item=layout name=layout}
                    <li data-layout-id="{$layout.id_layout|escape:'htmlall':'UTF-8'}" class="tmlist-group-item {if $layout.status == 1}active{/if}">
                        <a href="#"><i class="icon-check {if !$layout.status} hidden{else} visible{/if}"></i> {$layout.layout_name|escape:'htmlall':'UTF-8'}</a>
                    </li>
                {/foreach}
            {/if}
        </ul>
    </div>
    <div class="tmlist-layout-buttons">
...
<input type="hidden" name="tmml_id_hook" value="{$content.id_hook|escape:'htmlall':'UTF-8'}" />