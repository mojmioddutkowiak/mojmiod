...
class TmProductVideos extends Module
{
    ...
    public function hookActionProductUpdate()
    {
        ...
        foreach (Language::getLanguages() as $lang) {
            if (!Tools::isEmpty(trim(Tools::getValue('video_link_'.$lang['id_lang'])))
                ...
                && !Validate::isUrl(trim(Tools::getValue('video_link_'.$lang['id_lang'])))) {
                $this->context->controller->errors[] = Tools::displayError(sprintf($this->l('Video path/url is invalid for language - %s. It will not be saved.'), $lang['iso_code']));
            }
        }
            ...
            if (!Tools::isEmpty(trim(Tools::getValue('video_link_'.$lang['id_lang'])))
                ...
                if ((!Tools::isEmpty(trim(Tools::getValue('video_name_'.$lang['id_lang'])))
                    ...
                    && !Validate::isCleanHtml(Tools::getValue('video_description_'.$lang['id_lang']), (int)Configuration::get('PS_ALLOW_HTML_IFRAME')))) {
                        $errors[] = Tools::displayError(sprintf($this->l('Invalid content. Video wasn\'t saved for this language - %s'), $lang['iso_code']));
                } else {
    ...
    {
        $result = Db::getInstance()->ExecuteS('
            SELECT pvl.`link`, pvl.`cover_image`, pvl.`name`, pvl.`description`, pvl.`id_lang`, pvl.`sort_order`, pvl.`status`, pv.`id_video`
